...
class LaneDetector:
    def __init__(self, vertices, road_horizon, prob_hough=True):
    ...
    def detect(self, frame):
        ...
        #roi = self.roi_(img, self.vertices)
        blur = cv2.medianBlur(roi, 5)
        contours = cv2.Canny(blur, 60, 120)
        #cv2.imshow('tmp1',contours)

        if self.prob_hough:
            lines = cv2.HoughLinesP(contours, 1, np.pi/180, self.vote, minLineLength=30, maxLineGap=300)
        else:
        ...
        if lines is not None:
            ...
            for l in lines:
                ...
                for x1, y1, x2, y2 in l:
                    ...
                    if theta > self.roi_theta and theta < 0.5:  # ignore lines with a small angle WRT horizon
